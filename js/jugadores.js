...
const url =
  "https://cors-anywhere.herokuapp.com/https://scaloneta-api.vercel.app/jugadores";
let section = document.querySelector("section");
/*fetch(url)
.then((response) => response.json())
.then((data) => {
  let jugadores = data.map((jugador) => `<div class="col-12 col-md-4 p-3 float-start"><div class="card" >
  ...
  <div class="card-body">
    ...
    <p>Fecha de nacimiento: ${jugador.fecha}</p> 
    </p>
  </div>
</div></div>`).join().replaceAll(",", "")
})*/
...
let jugadores = [
  ...
  },
  {
    ...
    dorsal: 15,
    debut: 5,
    "stats-fisico": { peso: 70, altura: 1.71 },
  },
  {
    id: 23,
    name: "Ángel Di María",
  ...
  },
  {
    ...
    dorsal: 16,
    debut: 2022,
    "stats-fisico": { peso: 63, altura: 1.71 },
  },
  {
    id: 25,
    name: "Paulo Dybala",
...
let jugadoresMap = jugadores.map(
  (jugador) => `<div class="col-12 col-lg-6 col-xl-4 p-2 float-start "><div class="card">
<div class="img-wrapper">
  <span class="span">${jugador.dorsal}</span>
<img src="https://julioavantt.github.io/guayerd-project-images/img/${jugador.dorsal}.jpg" class="card-img-top" alt=""> 
</div>
<div class="card-body">
  <h5 class="card-title">${jugador.name}</h5>
  <p class="card-text">
  <p class="p">Edad: ${jugador.edad} </p>
  <p class="p">Posición: ${jugador.posicion}</p> 
  <p class="p">Peso: ${jugador["stats-fisico"].peso}</p> 
  <p class="p">Altura: ${jugador["stats-fisico"].altura}</p> 
  <p class="p">Equipo actual: ${jugador["equipo-actual"]}</p> 
  <p class="p">Fecha de nacimiento: ${jugador["fecha-nacimiento"]}</p> 
  </p>
</div>
</div></div>`
);

section.innerHTML = jugadoresMap.join().replaceAll(",", "");
